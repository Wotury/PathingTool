...
function newGridSize(width, height, tilesize){
	let canvas = document.getElementById(HTML_CANVAS);
	simMovementsInput = document.getElementById(HTML_RUNNER_MOVEMENTS);
	simMovementsInput.onkeypress = function (e) {
		if (e.key === " ") {
	...
	simSetRunning(false);
	rInit(canvas, width*tilesize, height*tilesize);
	rrInit(tilesize);
	mInit(mEMPTY, width, height);
	ruInit(5);
	simReset();
	window.onkeydown = simWindowOnKeyDown;
	canvas.onmousedown = simCanvasOnMouseDown;
...
function simStartStopButtonOnClick() {
	if (simIsRunning) {
		...
		} else if (mCurrentMap === mWAVE_1_TO_9){
...
function simWindowOnKeyDown(e) {
	...
	let xTile = Math.trunc((mousePosition.x - canvasRect.left) / rrTileSize);
	let yTile = Math.trunc((canvasRect.bottom - 1 - mousePosition.y) / rrTileSize);
	if(e.key === "f"){
		if(mCurrentMap[xTile + yTile * simGridSizeX] === "0x100")
			mCurrentMap[xTile + yTile * simGridSizeX] = ""
		else mCurrentMap[xTile + yTile * simGridSizeX] = "0x100";
		simDraw();
	}
	if(e.key === "j"){
		if(mCurrentMap[xTile + yTile * simGridSizeX] === "0x80")
			mCurrentMap[xTile + yTile * simGridSizeX] = ""
		else mCurrentMap[xTile + yTile * simGridSizeX] = "0x80";
		simDraw();
	}
	if(e.key === "i"){
		if(mCurrentMap[xTile + yTile * simGridSizeX] === "0x2")
			mCurrentMap[xTile + yTile * simGridSizeX] = ""
		else mCurrentMap[xTile + yTile * simGridSizeX] = "0x2";
		simDraw();
	}
	if(e.key === "k"){
		if(mCurrentMap[xTile + yTile * simGridSizeX] === "0x20")
			mCurrentMap[xTile + yTile * simGridSizeX] = ""
		else mCurrentMap[xTile + yTile * simGridSizeX] = "0x20";
		simDraw();
	}
	if(e.key === "l"){
		if(mCurrentMap[xTile + yTile * simGridSizeX] === "0x8")
			mCurrentMap[xTile + yTile * simGridSizeX] = ""
		else mCurrentMap[xTile + yTile * simGridSizeX] = "0x8";
		simDraw();
	}
...
function simWaveSelectOnChange(e) {
	if (simWaveSelect.value === "10") {
		...
		simInit();
		mInit(mWAVE10, 64, 48);
	} else if (simWaveSelect.value === "0"){
		simGridSizeX = 64;
		simGridSizeY = 48;
		simInit();
		mInit(mEMPTY, 64, 48);
	} else if (simWaveSelect.value === "11"){
		simGridSizeX = 32;
		simGridSizeY = 32;
		newGridSize(32, 32, 20);
		mInit(mEMPTY, 32, 32);
	} else if (simWaveSelect.value === "12"){
		simGridSizeX = 16;
		simGridSizeY = 16;
		newGridSize(16, 16, 35);
		mInit(mEMPTY, 16, 16);
	} else if (simWaveSelect.value === "13"){
		simGridSizeX = 10;
		simGridSizeY = 10;
		newGridSize(10, 10, 60);
		mInit(mEMPTY, 10, 10);
	} else if (simWaveSelect.value === "14"){
		simGridSizeX = 9;
		simGridSizeY = 9;
		newGridSize(9, 9, 65);
		mInit(mEMPTY, 9, 9);
	} else if (simWaveSelect.value === "15"){
		simGridSizeX = 8;
		simGridSizeY = 8;
		newGridSize(8, 8, 70);
		mInit(mEMPTY, 8, 8);
	} else if (simWaveSelect.value === "16"){
		simGridSizeX = 5;
		simGridSizeY = 5;
		newGridSize(5, 5, 100);
		mInit(mEMPTY, 5, 5);
...
const mEMPTY = ((new Array(2015).fill(0)).concat([0])).concat(new Array(2080).fill(0));
